...
/**
 * Http Response result process class, represents an HTTP 1.0 / 1.1 response message. It
...
class HttpResponse{
	...
	public function __construct($code, array $headers, $body = null, $version = '1.1', $message = null) {
		...
		if (self::responseCodeAsText ( $code ) === null) {
			//log_error ( "{$code} is not a valid HTTP response code" );
		}
		...
		foreach ( $headers as $name => $value ) {
			if (is_int ( $name )) {
				...
				if (count ( $header ) != 2) {
					//log_error ( "'{$value}' is not a valid HTTP header" );
				}
		...
		if (! preg_match ( '|^\d\.\d$|', $version )) {
			//log_error( "Invalid HTTP response version: $version" );
		}
	...
	public static function decodeChunkedBody($body) {
		...
		//while ( trim ( $body ) ) {
			...
			//curl针对chunked类型响应,会当作一个整体处理,返回的body中不会存在chunked响应标识.所以此处
			//针对chunked响应不作处理
			//if (! preg_match ( "/^([\da-fA-F]+)[^\r\n]*\r\n/sm", $body, $m )) {
			//	litb_log_warning( "Error parsing body - doesn't seem to be a chunked message" );
			//	litb_log_warning( "直接返回 body实体" );
				//return false;
				return $body;
			//}
	...
	public static function decodeGzip($body) {
		if (! function_exists ( 'gzinflate' )) {
			//log_error( 'zlib extension is required in order to decode "gzip" encoding' );
			return false;
		}
	...
	public static function decodeDeflate($body) {
		if (! function_exists ( 'gzuncompress' )) {
			//log_error( 'zlib extension is required in order to decode "deflate" encoding' );
			return false;
		}
